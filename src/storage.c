...
User users[MAX_USERS]; int users_count=0;
Project projects[MAX_PROJECTS]; int projects_count=0;
Membership members[MAX_MEMBERS]; int members_count=0;
Task tasks[MAX_TASKS]; int tasks_count=0;
Comment comments[MAX_COMMENTS]; int comments_count=0;

// ---- helpers parse rất nhẹ ----
static int extract_int(const char *line, const char *key, int *out){
    // tìm `"key":number`
    char pat[64]; sprintf(pat, "\"%s\":", key);
...
static int load_users(){
    ...
    if(!f) return 0; // file chưa có coi như rỗng
    char line[512];
    while(fgets(line,sizeof(line),f)){
        if(users_count>=MAX_USERS) break;
        User u; memset(&u,0,sizeof(u));
        if(!extract_int(line,"id",&u.id)) continue;
...
// ---- id generators ----
int next_user_id(){ return users_count==0?1:users[users_count-1].id+1; }
int next_project_id(){ return projects_count==0?1:projects[projects_count-1].id+1; }
int next_task_id(){ return tasks_count==0?1:tasks[tasks_count-1].id+1; }
int next_comment_id(){ return comments_count==0?1:comments[comments_count-1].id+1; }
...
int count_project_by_name(const char *name){
    ...
    if(!name) return 0;
    for(int i=0;i<projects_count;i++){
        if(strcmp(projects[i].name, name)==0) cnt++;
    }
    return cnt;
}
...
static int load_projects(){
    ...
    char line[512];
    while(fgets(line,sizeof(line),f)){
        if(projects_count>=MAX_PROJECTS) break;
        Project p; memset(&p,0,sizeof(p));
        if(!extract_int(line,"id",&p.id)) continue;
...
static int load_members(){
    ...
    char line[256];
    while(fgets(line,sizeof(line),f)){
        if(members_count>=MAX_MEMBERS) break;
        Membership m; memset(&m,0,sizeof(m));
        if(!extract_int(line,"project_id",&m.project_id)) continue;
        extract_int(line,"user_id",&m.user_id);
        int roleInt=1; extract_int(line,"role",&roleInt);
        m.role = (Role)roleInt;
...
static int load_tasks(){
    ...
    char line[1024];
    while(fgets(line,sizeof(line),f)){
        if(tasks_count>=MAX_TASKS) break;
        Task t; memset(&t,0,sizeof(t));
        if(!extract_int(line,"id",&t.id)) continue;
        ...
        int st=0; extract_int(line,"status",&st);
...
static int load_comments(){
    ...
    char line[1024];
    while(fgets(line,sizeof(line),f)){
        if(comments_count>=MAX_COMMENTS) break;
        Comment c; memset(&c,0,sizeof(c));
        if(!extract_int(line,"id",&c.id)) continue;
